...
class SearchInstance {
    ...
    runSearch() {
        let flags = SearchEnv.opts.match(/(?:(?:'[^']+')|\S)+/g) || [];
        if (SearchEnv.engine !== 'pt') {
            flags.push('-n');
        }
    ...
    startCSearch(flags) {
        ...
        const opts = SearchEnv.engine === 'csearch' ? { env: { CSEARCHINDEX: indexPath, PATH: process.env.PATH } } : undefined;
